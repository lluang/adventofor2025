...
profitability_df <- read.csv("../data/segment_profitability_means.csv", stringsAsFactors = TRUE, row.names = 1)
...
```{r}
#| label: define-model
# Source the constraints from the external script
...
model <- MIPModel() %>%
  ...
  # -- Objective Function --
  set_objective_net_profit(profitability, cost_orig, cost_sell, segment_ids) %>%
  
  # -- Constraints --
  add_flow_balance_constraint(exposure, segment_ids) %>%
  add_portfolio_risk_weight_constraint(risk_weight, segment_ids) %>%
  add_global_growth_constraint(total_portfolio_exposure, segment_ids)

```

```{r}
#| label: add-segment-constraints
# Asset Class Growth and Shrink Limits
model <- add_asset_exposure_constraints(model, segments_df, asset_ids, max_inc, max_dec, asset_current_total)
